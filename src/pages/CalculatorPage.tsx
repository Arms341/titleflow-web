...
export default function CalculatorPage() {
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => api.get('/calculators/').then(r => r.data),
  });

  if (isLoading) return <LoadingSpinner />;
  if (error) return <div className="p-4 text-red-600">Error loading calculators</div>;

  const items: CalculatorItem[] = Array.isArray(data) ? data : data?.items ?? [];
  const columns = ['id', 'name'];

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Calculators</h1>
